...
export function renderHome() {
  ...
  function renderizarprodutosEmDestaque(categoriaSelecionada = "todos") {
    ...
    produtosEmDestaqueFiltrados.forEach(produto => {
      ...
      if (categoria === "roupa") {
        // Mostrar bolinhas de variação de cor
      ...
      } else if (categoria === "equipamento" || categoria === "acessorio") {
        // Contêiner para a imagem e setas
        const imagemContainer = document.createElement('div');
        imagemContainer.classList.add('produto-imagem-container');
      
        // Adiciona as setas laterais
        ...
        setaEsq.addEventListener('click', () => {
          imagemAtual = (imagemAtual - 1 + produto.variacoes.length) % produto.variacoes.length;
          imagemPrincipal.src = produto.variacoes[imagemAtual].imagem;
        });
      
        setaDir.addEventListener('click', () => {
          imagemAtual = (imagemAtual + 1) % produto.variacoes.length;
          imagemPrincipal.src = produto.variacoes[imagemAtual].imagem;
        });
      
        // Adiciona a imagem e as setas ao contêiner
        imagemContainer.appendChild(imagemPrincipal);
        imagemContainer.appendChild(setaEsq);
        imagemContainer.appendChild(setaDir);
      
        // Adiciona o contêiner ao div do produto
